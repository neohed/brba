function BrBa(name) {
    ...
    function urlName() {
        var results = /\\?n=([^&#]*)/.exec(location.search);
    ...
    //todo idea to shorten code. Remove all symbols that are the first two chars of the name then
    // this.ChemicalSymbol = chemicalSymbol || name.subString(0, 2)
    var aNames = $('#n').val($('<div/>').text(urlName() || name).html()).val().split(' '),
        elements = [],
        //matchingElements = [],
        foundElements = [],
        foundUnique,
    ...
    elements.sort(function (a, b) { return a.ChemicalSymbol.length - b.ChemicalSymbol.length; });

    for (var indexName in aNames) {
        if (aNames.hasOwnProperty(indexName)) {
            ...
            while (aDataIndex-- && !foundUnique) {
                if (currentName.indexOf(elements[aDataIndex].ChemicalSymbol.toLowerCase()) !== -1) {
                    ...
                    for (var i = 0, foundElementsLength = foundElements.length; i < foundElementsLength; i++) {
